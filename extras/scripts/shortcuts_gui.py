...
import shortcuts_data
DATA = shortcuts_data.DataFunctions()

import shortcuts_library
LIBRARY = shortcuts_library.LibraryFunctions()
...
class GUI(xbmcgui.WindowXMLDialog):
                ...
                for i in range(0, 6):
                    ...
                    for path in paths:
                        ...
                        if path[1] == "New":
                            ...
                            shortcuts_data.UpgradeFunctions().upgrade_file(target.replace(".DATA.xml", ".shortcuts"))
                            shortcuts_data.UpgradeFunctions().upgrade_xmlfile(target.replace(".DATA.xml", ".shortcuts"))
    ...
    def onClick(self, controlID):
        ...
        if controlID == 310:
            ...
            if usePrettyDialog:
                w = shortcuts_library.ShowDialog("DialogSelect.xml", __cwd__, listing=backgroundPretty, windowtitle=__language__(32045))
                w.doModal()
        ...
        if controlID == 311:
            ...
            # Show the dialog
            w = shortcuts_library.ShowDialog("DialogSelect.xml", __cwd__, listing=thumbnailLabel, windowtitle="Select thumbnail")
            w.doModal()
        ...
        if controlID == 405 or controlID == 406 or controlID == 407 or controlID == 408 or controlID == 409 or controlID == 410:
            ...
            ui = shortcuts_gui.GUI("script-skinshortcuts.xml", __cwd__, "default", group=launchGroup, groupname=groupName)
            ui.doModal()
            del ui
